...
# Defining most of the variables we will end up using.
...
balance = 0
# CHANGE THE VARIABLE ABOVE TO YOUR BALANCE
nouns = []
workverb = []
...
# Using NLP to determine what transactions you are making.
def moneyManager():
    ...
    # For loop to check for key words
    ...
    # Writing transaction info to the transaction log file.
...
# Controls what code you interact with and when
def logLoop():
...
# Opens file with balance for the user to be used by the graph
def updateBalance():
    balanceFile = open(f'{user1}Balance.txt', 'a')
    balanceFile.write(str(balance) + '\n')


# Opens file with date for the user to be used by the graph
def updateTime():
    timeFile = open(f'{user1}Time.txt', 'a')
    timeFile.write(str(today1) + '\n')


# Formats the balance correctly for the
def balanceGraph():
...
# Retrieves balance from previous transaction to update current balance.
def findBalance():
    global balance
...
# Function that will calculate if your expenditures that you input are necessary
def product():
    months = 0
